...
public class WayOfDeliveryGUI 
{
	...
	public WayOfDeliveryGUI(MainGUI main) 
	{
		this.main = main;
		
		if (this.main.getClient().getWaysOfDelivery() instanceof Kurier) {
			jPanel = new JPanel();
			...
			JTextField miastoJTextField = new JTextField(((Kurier) this.main.getClient().getWaysOfDelivery()).getMiasto()); // Zeby uzytkownik nie musial ponownie wpisywac swoich danych
			...
			JTextField ulicaJTextField = new JTextField(((Kurier) this.main.getClient().getWaysOfDelivery()).getUlica()); // Zeby uzytkownik nie musial ponownie wpisywac swoich danych
			...
			JTextField kodPocztowyJTextField = new JTextField(((Kurier) this.main.getClient().getWaysOfDelivery()).getKodPocztowy()); // Zeby uzytkownik nie musial ponownie wpisywac swoich danych
		...
		else if(this.main.getClient().getWaysOfDelivery() instanceof Osobisty) {
			((Osobisty) this.main.getClient().getWaysOfDelivery()).setTomDt();
			...
			JLabel dataJLabel = new JLabel(((Osobisty) this.main.getClient().getWaysOfDelivery()).getDt() + " do konca dnia nastepnego w godzinach 8-20");
		...
		else if (this.main.getClient().getWaysOfDelivery() instanceof Paczkomat) {
			jPanel = new JPanel();
			BoxLayout boxLayout = new BoxLayout(jPanel, BoxLayout.Y_AXIS);
			jPanel.setLayout(boxLayout);
	...
	class ConfirmData implements ActionListener
	{
		public void actionPerformed(ActionEvent event) 
		{
			...
			if (((Dostawa) main.getClient().getWaysOfDelivery()).isCorrectData(deliveryInfo)) {
				((Dostawa) main.getClient().getWaysOfDelivery()).setDeliveryInfo(deliveryInfo);
	...
	class ConfirmMiasto implements ActionListener
	{
		public void actionPerformed(ActionEvent event) 
		{
			...
			if (((Kurier) main.getClient().getWaysOfDelivery()).isCorrectMiasto(miast)) {
				if(JOptionPane.showConfirmDialog(null, "Czy to na pewno jest twoje miasto? " + miast, "Potwierdz dane", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
					((Kurier) main.getClient().getWaysOfDelivery()).setMiasto(miast);
